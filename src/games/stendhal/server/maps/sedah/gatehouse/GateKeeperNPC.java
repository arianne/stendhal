...
/**
 * Builds a gatekeeper NPC 
 * Bribe him with at least 300 money to get the key for the Sedah city walls.
 * He stands in the doorway of the gatehouse till the interior is made.
 * @author kymara
 */
public class GateKeeperNPC implements ZoneConfigurator {
	...
	private void buildNPC(StendhalRPZone zone) {
			...
			protected void createDialog() {
				...
				addReply("bribe", null, new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text,
					    ...
					    // This bit is just incase the player says 'bribe X potatoes', not money
					    if (words.length > 2 && !words[2].trim().toLowerCase().equals("money") ){
						...
						try {
							...
							if (player.isEquipped("money",amount)){
							    ...
							    Item key = StendhalRPWorld.get().getRuleManager()
								.getEntityManager().getItem("sedah_gate_key");
							    player.equip(key, true);
							}
							else { 
							    // player bribed enough but doesn't have the cash
							    engine.say("Criminal! You don't have " + amount + " money!");
							}
						    }
						}
		...
		/* We don't seem to be using the recruiter images that lenocas made for the Fado Raid area 
		   so I'm going to put him to use here. If the raid part ever gets done, this image can change.*/
