...
public class ExperiencedWarriorNPC extends SpeakerNPCFactory {
	...
	/**
	 * cost of the information for players. Final cost is:
	 * INFORMATION_BASE_COST + creatureLevel * INFORMATION_COST_LEVEL_FACTOR
	 */
	...
	/**
	 * literals for probabilities. %s is replaced with item description (name and amount)
	 */
	static Map<Double, String> probabilityLiterals = null;
	/**
	 *  literal for item amounts
	 * %s is replaced with singular item name, %a with "a/an item name" depending on the item name
	 */
	static Map<Integer, String> amountLiterals = null;

	/**
	 * literal for how dangerous a creature is based on the percentual difference to player level
	 * %s is replaced with singular creature name, %S with plural
	 */
	static Map<Double, String> dangerLiterals = null;

	/**
	 * literals for line starts. %s is replaced with singular creature name,
	 * %S plural
	 */
	static final String[] LINE_STARTS = new String[] {
		"Oh yes I know %s!",
	...
	static {
		...
		probabilityLiterals.put(0.00000001, "maybe %s too, but I've only heard about that");
		...
		dangerLiterals.put(15.0, "%s is probably lethal for you, don't try to kill one!");
	 ...
	 */
	static final String[] CARRY_TEXTS = new String[] {
		"They carry %1.",
		"Dead ones have %1.",
		"The corpses contain %1."
	};
	...
	static final String[] LOCATION_TEXTS = new String[] {
		"I have seen them %1.",
		"You should be able to find them %1.",
	 ...
	 */
	static final String[] LOCATION_UNKNOWN_TEXTS = new String[] {
			"I don't know of any place where you could find %1."
	};

	static CreatureInfo creatureInfo = new CreatureInfo(
			probabilityLiterals,
			amountLiterals,
			dangerLiterals,
			LINE_STARTS,
	...
	protected void createDialog(SpeakerNPC npc) {
		...
		npc.addGreeting();
		npc.setLevel(368);
		npc.setDescription("You see " + npc.getName() + ", the mighty warrior and defender of Semos.");
		...
		npc.add(ConversationStates.ATTENDING,
				...
				"Which creature you would like to hear more about?",
		        null);
		...
		npc.add(ConversationStates.QUESTION_1,
		        ...
		        new SpeakerNPC.ChatAction() {
					...
					private int getCost(Player player, DefaultCreature creature) {
						return (int) (INFORMATION_BASE_COST + INFORMATION_COST_LEVEL_FACTOR * creature.getLevel());
					}
		        }
		);

		npc.add(ConversationStates.BUY_PRICE_OFFERED, ConversationPhrases.YES_MESSAGES, null,
		...
		npc.add(ConversationStates.BUY_PRICE_OFFERED, ConversationPhrases.NO_MESSAGES, null,
		        ConversationStates.IDLE, "Ok, come back if you're interested later.", null);

		npc.addGoodbye("Farewell and godspeed!");
	}

	private String getCreatureInfo(final Player player, final String creatureName) {
		String result = null;
		DefaultCreature creature;
		EntityManager manager = StendhalRPWorld.get().getRuleManager().getEntityManager();
		if((creature = manager.getDefaultCreature(creatureName)) != null) {
