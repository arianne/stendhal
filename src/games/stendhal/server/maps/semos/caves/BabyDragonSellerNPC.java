	...
	private void buildHouseArea(StendhalRPZone zone) {
		...
		SpeakerNPC npc = new SpeakerNPC("Terry") {
			...
			protected void createDialog() {
				addGreeting(null, new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, Sentence sentence,
						...
						if (player.hasQuest(QUEST_SLOT)) {
							...
							long timeRemaining = (Long.parseLong(player.getQuest(QUEST_SLOT)) + delay)
							...
							if (player.hasPet()) {
								// we don't want him to give a dragon if player
								// already has a pet
							...
							babydragon.setPosition(engine.getX(),
									engine.getY() + 1);
					...
					public void fire(Player player, Sentence sentence,
						...
						if (!player.hasPet()) {
							if (player.isEquipped("mythical_egg")) {
								...
								engine.say("Ok, I'll take your egg and hatch it in one of these nesting boxes. Come back in "
										+ 7
										+ " days and you should be the proud owner of a new born baby dragon.");
								player.setQuest(
										QUEST_SLOT,
							...
							// there's actually also a check for this when the
							// egg is hatched,
							// but we might as well warn player here that they
							// wouldn't be allowed two.
				...
				addReply(
						"food",
						"Baby dragons feed on meat and ham. Their particular favourite is pizza, if you can get it.");
				addReply(
						"care",
		...
		// Also put a dragon in the caves (people can't Own it as it is behind
		// rocks)
