...
public class ToysCollector extends AbstractQuest {
	...
	private void step_3() {
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				...
				"Hello! I'm still bored. Did you bring me toys?", null);

		// player says he has a required toy with him
		npc.add(ConversationStates.ATTENDING, ConversationPhrases.YES_MESSAGES,
				null, ConversationStates.QUESTION_1, "What did you bring?!",
				null);

		for (String toy : neededToys) {
			npc.add(ConversationStates.QUESTION_1, toy, null,
							...
							if (missing.contains(text)) {
								if (player.drop(text)) {
									...
									String doneText = player
											.getQuest("toys_collector");
									player.setQuest("toys_collector", doneText
											+ ";" + text);
									...
									if (missing.size() > 0) {
										...
										StackableItem pie = (StackableItem) StendhalRPWorld
												.get().getRuleManager()
									...
									engine
											.say("Hey! It's bad to lie! You don't have "
													+ Grammar.a_noun(text)
													+ " with you.");
								}
							} else {
		...
		npc.add(ConversationStates.QUESTION_1, "",
				...
				}, ConversationStates.QUESTION_1, "That's not a good toy!",
				null);
				...
				.add(
						...
						new SpeakerNPC.ChatCondition() {
							...
							public boolean fire(Player player, String text,
									SpeakerNPC engine) {
								return !player
										.isQuestCompleted("toys_collector");
							}
						},
						ConversationStates.ATTENDING,
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				...
				"Hi! I'm busy playing with my toys, no grown ups allowed.",
				null);
	}
