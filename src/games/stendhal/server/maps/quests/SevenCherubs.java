					...
					new SpeakerNPC.ChatAction() {
						...
						public void fire(Player player, String text,
							...
							if (!list.contains(engine.getName())) {
								...
								if (left > 0) {
									engine
											.say("Well done! You only need to find "
													+ (7 - list.size())
													+ " more. Fare thee well!");
									player.addXP((7 - left + 1) * 200);
								} else {
									engine
											.say("Thou hast proven thyself brave enough to bear this mighty relic!");
								...
								if (left > -1) {
									engine
											.say("Seek out the other cherubim to get thy reward!");
								} else {
									engine
