...
public class SevenCherubs extends AbstractQuest {
	...
	static class CherubNPC extends SpeakerNPC {
		...
		protected void createDialog() {
			add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES, null, ConversationStates.IDLE, null,
			        new SpeakerNPC.ChatAction() {
				        ...
				        public void fire(Player player, String text, SpeakerNPC engine) {
					        ...
					        if (!list.contains(engine.getName())) {
						        player.setQuest(QUEST_SLOT, npcDoneText + ";" + engine.getName());

						        player.setHP(player.getBaseHP());
						        player.healPoison();

						        if (left > 0) {
							        engine.say("Well done! You only need to find " + (7 - list.size())
							                + " more. Fare thee well!");
							         ...
							         * 
							         */
							        String[] items = { "golden_boots", "golden_armor", /* "fire_sword",
						        ...
						        if (left > -1) {
							        ...
							        engine
							                .say("Thou hast sought and found each of the seven cherubim! Now, mighty art thou with the rewards so earn'd.");
						        }
					        }
