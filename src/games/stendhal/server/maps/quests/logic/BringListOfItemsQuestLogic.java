...
/**
 * An abstract quest which is based on bringing a list of items to an NPC. The
 * NPC keeps track of the items already brought to him.
 */
public class BringListOfItemsQuestLogic {
	/**
	 * the concrete quest information (which items?, which npc?, what does it
	 * say?)
	 */
	 ...
	 */
	protected List<String> getListOfStillMissingItems(Player player,
			boolean hash) {
	...
	protected void welcomeNewPlayer() {
		concreteQuest.getNPC().add(ConversationStates.IDLE,
				...
				concreteQuest.welcomeBeforeStartingQuest(), null);
	}
	...
	protected void tellAboutQuest() {
		List<String> questTrigger = new LinkedList<String>(
		...
		concreteQuest.getNPC().add(ConversationStates.ATTENDING, questTrigger,
				new QuestNotStartedCondition(concreteQuest.getSlotName()),
	...
	protected void acceptQuest() {
		concreteQuest.getNPC().add(
				...
				concreteQuest.respondToQuestAcception(),
				new SetQuestAndModifyKarmaAction(concreteQuest.getSlotName(),
						"", concreteQuest.getKarmaDiffForQuestResponse()));
	}
			...
			concreteQuest.getNPC().add(ConversationStates.QUEST_OFFERED,
					...
					new SpeakerNPC.ChatAction() {
						...
						public void fire(Player player, Sentence sentence,
								SpeakerNPC engine) {
							List<String> missingItems = getListOfStillMissingItems(
									player, false);
	...
	protected void listMissingItems() {
		...
		// yes. In this case it is ignored during the "giving items" state
		// because
	...
	protected void playerWantsToGiveItems() {
		int[] states = new int[] { ConversationStates.ATTENDING,
				ConversationStates.QUESTION_1 };
		concreteQuest.getNPC().add(states, ConversationPhrases.YES_MESSAGES,
				new QuestActiveCondition(concreteQuest.getSlotName()),
				ConversationStates.QUESTION_1,
				concreteQuest.askForItemsAfterPlayerSaidHeHasItems(), null);
	}
	...
	protected void offerItem() {
		concreteQuest.getNPC().add(ConversationStates.QUESTION_1,
				concreteQuest.getNeededItems(), null,
				ConversationStates.QUESTION_1, null,
				new SpeakerNPC.ChatAction() {
					@Override
					public void fire(Player player, Sentence sentence,
						...
						List<String> missing = getListOfStillMissingItems(
								player, false);
						...
						String doneText = player.getQuest(concreteQuest.getSlotName());
						player.setQuest(concreteQuest.getSlotName(), doneText
								+ ";" + item);
	...
	protected void welcomeKnownPlayer() {
		concreteQuest.getNPC().add(ConversationStates.IDLE,
				...
				new QuestActiveCondition(concreteQuest.getSlotName()),
				ConversationStates.ATTENDING,
				concreteQuest.welcomeDuringActiveQuest(), null);
	}
	...
	protected void welcomePlayerAfterQuest() {
		if (concreteQuest.shouldWelcomeAfterQuestIsCompleted()) {
			concreteQuest.getNPC().add(ConversationStates.IDLE,
					...
					ConversationStates.ATTENDING,
					concreteQuest.welcomeAfterQuestIsCompleted(), null);
		}
	}
