...
public class WeaponsCollector extends AbstractQuest {
	...
	private static final List<String> neededWeapons = Arrays.asList("axe", "bardiche", "battle_axe", "biting_sword",
	        "broadsword", "flail", "hammer_+3", "halberd", "hammer", "katana", "mace_+2", "scimitar", "scythe",
	        "twoside_axe", "war_hammer");
	...
	private void step_1() {
		...
		npc.add(ConversationStates.ATTENDING, "collection", new SpeakerNPC.ChatCondition() {

			@Override
			public boolean fire(Player player, String text, SpeakerNPC engine) {
				return player.hasQuest("weapons_collector") && !player.isQuestCompleted("weapons_collector");
			}
			...
			public void fire(Player player, String text, SpeakerNPC engine) {
				...
				engine.say("There " + Grammar.isare(needed.size()) + " "
				        + Grammar.quantityplnoun(needed.size(), "weapon") + " still missing from my collection: "
				        + Grammar.enumerateCollection(needed) + ". Do you have anything of that nature with you?");
			}
		});

		// player says he doesn't have required weapons with him
		npc.add(ConversationStates.QUESTION_1, "no", null, ConversationStates.IDLE, null, new SpeakerNPC.ChatAction() {
		...
		npc.add(ConversationStates.QUESTION_1, ConversationPhrases.YES_MESSAGES, null, ConversationStates.QUESTION_1,
		        "What is it that you found?", null);

		for (String weapon : neededWeapons) {
			npc.add(ConversationStates.QUESTION_1, weapon, null, ConversationStates.QUESTION_1, null,
					        ...
					        if (missing.contains(text)) {
						        if (player.drop(text)) {
							        ...
							        if (missing.size() > 0) {
								        ...
								        Item iceSword = StendhalRPWorld.get().getRuleManager().getEntityManager()
								                .getItem("ice_sword");
	...
	private void step_3() {
		...
		}, ConversationStates.ATTENDING, "Welcome back. I hope you have come to help me with my #collection.", null);
