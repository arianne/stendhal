				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, Sentence sentence,
						...
						if (!player.hasQuest(QUEST_SLOT)) {
							...
							npc.say("Welcome back! Did you look up the famous quote by "
									+ name + "?");
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, Sentence sentence,
						...
						if (player.isQuestCompleted(QUEST_SLOT)) {
							...
							String name = player.getQuest(QUEST_SLOT);
							npc.say("I already asked you for a favor already! Have you already looked up the famous quote by "
									+ name + "?");
							npc.setCurrentState(ConversationStates.QUESTION_1);
						} else {
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, Sentence sentence,
						...
						if (sentence.toString().equalsIgnoreCase(quote)) {
							...
							Item fishingRod = StendhalRPWorld.get().getRuleManager().getEntityManager().getItem(
									"fishing_rod");
