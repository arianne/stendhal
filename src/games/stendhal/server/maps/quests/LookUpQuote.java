		...
		fisherman.add(ConversationStates.IDLE,
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						if (!player.hasQuest(QUEST_SLOT)) {
							npc
									.say("Hello newcomer! I can #help you on your way to become a real fisherman!");
						} else if (!player.isQuestCompleted(QUEST_SLOT)) {
							String name = player.getQuest(QUEST_SLOT);
							npc
									.say("Welcome back! Did you look up the famous quote by "
											+ name + "?");
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						if (player.isQuestCompleted(QUEST_SLOT)) {
							...
							String name = player.getQuest(QUEST_SLOT);
							npc
									.say("I already asked you for a favor already! Have you already looked up the famous quote by "
											+ name + "?");
							npc.setCurrentState(ConversationStates.QUESTION_1);
						} else {
							npc
									.say("Well, I once had a book with quotes of famous fishermen, but I lost it. And now I cannot remember a certain quote. Can you look it up for me?");
						}
					}
				});
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						...
						if (text.equalsIgnoreCase(quote)) {
							npc
									.say("Oh right, that's it! How could I forget this? Here, take this handy fishing rod as an acknowledgement of my gratitude!");
							...
							npc
									.say("I think you made a mistake. Come back if you can tell me the correct quote.");
