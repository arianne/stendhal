...
public class VampireSword extends AbstractQuest {
	...
	private void prepareQuestOfferingStep() {
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						if (!player.hasQuest(QUEST_SLOT)) {
							...
							npc
									.say("What are you bothering me for now? You've got your sword, go and use it!");
						} else {
							npc
									.say("Why are you bothering me when you haven't completed your quest yet?");
						}
					}
				});
		...
		npc.add(ConversationStates.QUEST_OFFERED,
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						npc
								.say("Then you need this #goblet. Take it to the Semos #Catacombs.");
	...
	private void prepareGobletFillingStep() {
		...
		npc
				.addReply(
						Arrays.asList("blood", "vampirette_entrails",
								"bat_entrails"),
	...
	private void prepareForgingStep() {
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						if (!player.isEquipped("iron", REQUIRED_IRON)) {
							npc
									.say("You have battled hard to bring that goblet. I will use it to #forge the vampire sword");
						} else {
							player.drop("goblet");
							player.drop("iron", REQUIRED_IRON);
							npc
									.say("You've brought everything I need to make the vampire sword. Come back in "
											+ REQUIRED_MINUTES
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						npc
								.say("I hope you didn't lose your goblet! Do you need another?");
					}
				});
		...
		npc.add(ConversationStates.QUESTION_1,
				...
				ConversationStates.IDLE, null, new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						npc
								.say("You stupid ..... Be more careful next time. Bye!");
						Item emptygoblet = StendhalRPWorld.get()
								.getRuleManager().getEntityManager().getItem(
										"empty_goblet");
						player.equip(emptygoblet, true);
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				new SpeakerNPC.ChatCondition() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						...
						if (timeRemaining > 0L) {
							npc
									.say("I haven't finished forging the sword. Please check back in "
											+ TimeUtil
