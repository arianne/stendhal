...
public class VampireSword extends AbstractQuest {
	...
	private void step_1() {
		        ...
		        ConversationStates.QUEST_OFFERED, null, new SpeakerNPC.ChatAction() {
			        ...
			        public void fire(Player player, String text, SpeakerNPC engine) {
				        if (!player.hasQuest(QUEST_SLOT)) {
					        engine
					                .say("I can forge a powerful life stealing sword for you. You will need to go to the Catacombs below Semos Graveyard and fight the Vampire Lord. Are you interested?");
		...
		npc.add(ConversationStates.QUEST_OFFERED, "no", null, ConversationStates.IDLE,
		        "Oh, well forget it then. You must have a better sword than I can forge, huh? Bye.", null);

		npc.add(ConversationStates.ATTENDING, "Catacombs", null, ConversationStates.ATTENDING,
		        "The Catacombs of North Semos of the ancient #stories.", null);

		npc.add(ConversationStates.ATTENDING, "goblet", null, ConversationStates.ATTENDING,
		        "Go fill it with the blood of the enemies you meet in the #Catacombs.", null);
	...
	private void step_3() {
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES, new SpeakerNPC.ChatCondition() {
			...
			public boolean fire(Player player, String text, SpeakerNPC engine) {
				return player.hasQuest(QUEST_SLOT) && player.getQuest(QUEST_SLOT).equals("start")
				        && !player.isEquipped("goblet") && player.isEquipped("empty_goblet");
			}
			...
			public void fire(Player player, String text, SpeakerNPC engine) {
				engine
				        .say("Did you lose your way? The Catacombs are in North Semos. Don't come back without a full goblet! Bye!");
			}
		});
		...
		npc.add(ConversationStates.QUESTION_1, ConversationPhrases.YES_MESSAGES, null, ConversationStates.IDLE, null,
		...
		npc.add(ConversationStates.QUESTION_1, "no", null, ConversationStates.IDLE,
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES, new SpeakerNPC.ChatCondition() {
			...
			@Override
			public boolean fire(Player player, String text, SpeakerNPC engine) {
				return player.hasQuest(QUEST_SLOT) && player.getQuest(QUEST_SLOT).startsWith("forging;");
			}
		}, ConversationStates.IDLE, null, new SpeakerNPC.ChatAction() {
			...
			public void fire(Player player, String text, SpeakerNPC engine) {
				...
				engine
				        .say("I have finished forging the mighty Vampire Sword. You deserve this. Now i'm going back to work, goodbye!");
				player.addXP(5000);
		...
		npc.add(ConversationStates.QUEST_ITEM_BROUGHT, "forge", null, ConversationStates.QUEST_ITEM_BROUGHT,
		        "Bring me 10 #iron bars to forge the sword with. Don't forget to bring the goblet too.", null);

		npc.add(ConversationStates.QUEST_ITEM_BROUGHT, "iron", null, ConversationStates.IDLE,
		        "You know, collect the iron ore lying around and get it cast! Bye!", null);
