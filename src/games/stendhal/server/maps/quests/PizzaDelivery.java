...
public class PizzaDelivery extends AbstractQuest {
	...
	private static class CustomerData {
		...
		private CustomerData(String npcDescription, String flavor,
	...
	private void startDelivery(Player player, SpeakerNPC npc) {
		...
		npc
				.say("You must bring this "
						+ data.flavor
						+ " to #"
						+ name
						+ " within "
						+ Grammar
								.quantityplnoun(data.expectedMinutes, "minute")
						+ ". Say \"pizza\" so that "
						+ name
			...
			for (Item pizza : player.getAllEquipped("pizza")) {
				...
				if (data.flavor.equals(flavor)) {
					...
					if (player.hasQuest(QUEST_SLOT)) {
						...
						// slot).
						npc
								.say("Eek! This pizza is all dirty! Did you find it on the ground?");
					}
					return;
				}
			}
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						if (player.hasQuest(QUEST_SLOT)) {
							...
							if (isDeliveryTooLate(player)) {
								...
								npc
										.say("I see you failed to deliver the pizza to "
												+ customerName
