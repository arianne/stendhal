 ...
 * 
 * STEPS:
 * - Maerion asks you fix his dark elf problem
 * - You go kill at least a dark elf archer, captain, and thing
 * - The thing drops an amulet 
				...
				new SpeakerNPC.ChatCondition() {
					...
					public boolean fire(Player player, String text, SpeakerNPC engine) {
						return player.hasQuest("kill_dark_elves")
								&& player.getQuest("kill_dark_elves").equals("start");
					}
				},
					...
					public void fire(Player player, String text, SpeakerNPC engine) {
						if (player.hasKilled("dark_elf_archer") 
							...
							if(player.drop("amulet")){
								...
								Item emeraldring = StendhalRPWorld.get().getRuleManager().getEntityManager().getItem("emerald_ring");
							...
							// this happens if player has killed the thing but left the amulet somewhere else
							else{
