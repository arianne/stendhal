...
public class FindGhosts extends AbstractQuest {
	...
	private static final List<String> NEEDED_SPIRITS = Arrays.asList("Mary",
			"Ben", "Zak", "Goran");
		...
		npc.add(ConversationStates.ATTENDING,
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text,
						...
						if (!player.hasQuest("find_ghosts")) {
							engine
									.say("I feel so lonely. I only ever see creatures and alive people. If I knew about #spirits like me, I would feel better.");
						} else if (!player.isQuestCompleted("find_ghosts")) {
							engine
									.say("I want help to find other spirits like me. Please find them, then come back and tell me their names.");
							engine
									.setCurrentState(ConversationStates.ATTENDING);
						} else {
							engine
									.say("Thank you! I feel better now that I know the names of other spirits on Fauimoni.");
							engine
									.setCurrentState(ConversationStates.ATTENDING);
						}
					}
				...
				new SpeakerNPC.ChatCondition() {
					...
					public boolean fire(Player player, String text,
						...
						return player.hasQuest("find_ghosts")
								&& player.getQuest("find_ghosts").startsWith(
										"looking");
					}
			...
			npc.add(ConversationStates.QUESTION_1, name, null,
					...
					new SpeakerNPC.ChatAction() {
						...
						public void fire(Player player, String text,
							...
							String npcQuestText = player
									.getQuest("find_ghosts");
							String[] npcDoneText = npcQuestText.split(":");
							List<String> looking = Arrays.asList(npcDoneText[0]
									.split(";"));
							List<String> said = Arrays.asList(npcDoneText[1]
									.split(";"));
							String reply = "";
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				new SpeakerNPC.ChatCondition() {
					...
					public boolean fire(Player player, String text,
							SpeakerNPC engine) {
						return !player.hasQuest("find_ghosts")
								|| player.isQuestCompleted("find_ghosts");
					}
