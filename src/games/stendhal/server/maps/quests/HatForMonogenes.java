...
public class HatForMonogenes extends AbstractQuest {
	...
	private void createRequestingStep() {
		...
		monogenes.add(ConversationStates.ATTENDING, ConversationPhrases.QUEST_MESSAGES, null,
		        ConversationStates.QUEST_OFFERED, null, new SpeakerNPC.ChatAction() {
			        ...
			        public void fire(Player player, String text, SpeakerNPC npc) {
				        if (!player.isQuestCompleted("hat_monogenes")) {
					        npc
					                .say("Could you bring me a #hat to cover my bald head? Brrrrr! The days here in Semos are really getting colder...");
				        } else {
					        npc
					                .say("Thanks for the offer, good friend, but this hat will last me five winters at least, and it's not like I need more than one.");
					        npc.setCurrentState(ConversationStates.ATTENDING);
				        }
			        }
		        });

		monogenes.add(ConversationStates.QUEST_OFFERED, ConversationPhrases.YES_MESSAGES, null,
		        ConversationStates.ATTENDING, "Thanks, my good friend. I'll be waiting here for your return!",
	...
	private void createBringingStep() {
		...
		monogenes.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES, new SpeakerNPC.ChatCondition() {
			...
			public void fire(Player player, String text, SpeakerNPC npc) {
				if (player.isEquipped("leather_helmet")) {
					...
					npc
					        .say("Hey, my good friend, remember that leather hat I asked you about before? It's still pretty chilly here...");
					npc.setCurrentState(ConversationStates.ATTENDING);
				}
			}
		});
		...
		monogenes.add(ConversationStates.QUEST_ITEM_BROUGHT,
		        ...
		        }, ConversationStates.ATTENDING, "Bless you, my good friend! Now my head will stay nice and warm.",
		        new SpeakerNPC.ChatAction() {
		...
		monogenes
		        .add(ConversationStates.QUEST_ITEM_BROUGHT, ConversationPhrases.NO_MESSAGES, null,
		                ConversationStates.ATTENDING,
		                "I guess someone more fortunate will get his hat today... *sneeze*", null);
	}
