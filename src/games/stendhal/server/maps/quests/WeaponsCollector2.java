...
public class WeaponsCollector2 extends AbstractQuest {
	...
	private static final List<String> neededWeapons = Arrays.asList("morning_star", // fairly rare from glow_monster in haunted house
	...
	private void step_1() {
		...
		}, ConversationStates.ATTENDING, "Greetings, old friend. If you are willing, I have another #quest for you.",
		        null);

		npc.add(ConversationStates.ATTENDING, ConversationPhrases.QUEST_MESSAGES, new SpeakerNPC.ChatCondition() {
		...
		npc.add(ConversationStates.QUEST_2_OFFERED, ConversationPhrases.YES_MESSAGES, null,
		...
		npc.add(ConversationStates.QUEST_2_OFFERED, "no", null, ConversationStates.ATTENDING,
		        "Well, maybe someone else will happen by and help me.", null);

		// player asks what exactly is missing
		npc.add(ConversationStates.ATTENDING, "list", new SpeakerNPC.ChatCondition() {

			@Override
			public boolean fire(Player player, String text, SpeakerNPC engine) {
				return player.hasQuest("weapons_collector2") && !player.isQuestCompleted("weapons_collector2");
			}
			...
			public void fire(Player player, String text, SpeakerNPC engine) {
				...
				engine.say("There " + Grammar.isare(needed.size()) + " "
				        ...
				        + " still missing from my newest collection: " + Grammar.enumerateCollection(needed)
				        + ". Do you have anything like that with you?");
			}
		});

		// player says he doesn't have required weapons with him
		npc.add(ConversationStates.QUESTION_2, "no", null, ConversationStates.IDLE, null, new SpeakerNPC.ChatAction() {
		...
		npc.add(ConversationStates.QUESTION_2, ConversationPhrases.YES_MESSAGES, null, ConversationStates.QUESTION_2,
		        "What did you find?", null);

		for (String weapon : neededWeapons) {
			npc.add(ConversationStates.QUESTION_2, weapon, null, ConversationStates.QUESTION_2, null,
					        ...
					        if (missing.contains(text)) {
						        if (player.drop(text)) {
							        ...
							        if (missing.size() > 0) {
								        ...
								        Item lhandsword = StendhalRPWorld.get().getRuleManager().getEntityManager()
								                .getItem("l_hand_sword");
								        lhandsword.put("bound", player.getName());
								        player.equip(lhandsword, true);
								        Item rhandsword = StendhalRPWorld.get().getRuleManager().getEntityManager()
								                .getItem("r_hand_sword");
							        ...
							        engine.say("I may be old, but I'm not senile, and you clearly don't have "
							                + Grammar.a_noun(text) + ". What do you really have for me?");
						        }
					        } else {
	...
	private void step_3() {
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES, new SpeakerNPC.ChatCondition() {
			...
			public boolean fire(Player player, String text, SpeakerNPC engine) {
		        ...
		        "Welcome back. I hope you have come to help me with my latest #list of weapons.", null);
		...
		}, ConversationStates.ATTENDING, "Welcome! Thanks again for extending my collection.", null);
	}
