...
public class CleanStorageSpace extends AbstractQuest {
	...
	private void step_1() {
		...
		npc.add(ConversationStates.QUEST_OFFERED, "no", null, ConversationStates.ATTENDING,
		        "*sigh* Oh well, maybe someone else will be my hero...", new SpeakerNPC.ChatAction() {
	...
	private void step_3() {
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES, new SpeakerNPC.ChatCondition() {
			...
			public boolean fire(Player player, String text, SpeakerNPC engine) {
				return player.hasQuest("clean_storage") && player.getQuest("clean_storage").equals("start");
			}
		}, ConversationStates.QUEST_STARTED, null, new SpeakerNPC.ChatAction() {

			@Override
			public void fire(Player player, String text, SpeakerNPC engine) {
				if (player.hasKilled("rat") && player.hasKilled("caverat") && player.hasKilled("cobra")) {
