 ...
 * REPETITIONS:
 * - as many as desired
 * - Only possible to repeat once every ten minutes
 * 
 * @author kymara
 */
public class Soup extends AbstractQuest {

    private static final List<String> NEEDED_FOOD = Arrays.asList("carrot", "spinach", "courgette", "collard",
								  "salad", "onion", "cauliflower", "broccoli", "leek");
	...
	// player returns after finishing the quest (it is repeatable) after the time as finished
	npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
		new SpeakerNPC.ChatCondition() {
		    @Override
			public boolean fire(Player player, String text, SpeakerNPC npc) {
			// we don't set quest slot to done so we can't check this
		...
		ConversationStates.QUEST_OFFERED, "Hello again. Have you returned for more of my special soup?", null);

	// player returns after finishing the quest (it is repeatable) before the time as finished
	npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
		new SpeakerNPC.ChatCondition() {
		    @Override
			public boolean fire(Player player, String text, SpeakerNPC npc) {
	...
	npc.add(ConversationStates.QUEST_OFFERED,
		...
		new SpeakerNPC.ChatAction() {
			...
			npc.say("I need " + Grammar.quantityplnoun(needed.size(), "ingredient") +
	...
	npc.add(ConversationStates.QUEST_OFFERED,
		...
		new SpeakerNPC.ChatAction() {
		    @Override
			public void fire(Player player, String text, SpeakerNPC npc) {
			npc.say("You made a wise choice. Do you have anything I need already?");
	...
	npc.add(ConversationStates.QUEST_OFFERED,
		...
		"So will you fetch the ingredients?",
		null
		);
	...
	npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
		new SpeakerNPC.ChatCondition() {
		    @Override
			public boolean fire(Player player, String text, SpeakerNPC npc) {
			return player.hasQuest(QUEST_SLOT) && !player.getQuest(QUEST_SLOT).startsWith("done");
		    }
		},
	...
	npc.add(ConversationStates.QUESTION_1,
		...
		new SpeakerNPC.ChatAction() {
			...
			public void fire(Player player, String text, SpeakerNPC npc) {
			List<String> needed = missingFood(player, true);
			npc.say("I still need " + Grammar.quantityplnoun(needed.size(), "ingredient") +
