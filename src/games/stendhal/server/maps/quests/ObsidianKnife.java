...
public class ObsidianKnife extends AbstractQuest {
	...
	private void requestBookStep() {
		...
		npc.add(ConversationStates.QUEST_ITEM_BROUGHT,
				...
				new SetQuestAndModifyKarmaAction(QUEST_SLOT, "seeking_book",
						10.0));
	}
	...
	private void getBookStep() {
		...
		npc.add(ConversationStates.QUESTION_1, "", new NotCondition(
				new TriggerInListCondition(NAME.toLowerCase())),
				ConversationStates.QUESTION_1,
	...
	private void bringBookStep() {
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				new AndCondition(new QuestInStateCondition(QUEST_SLOT,
						"got_book"), new PlayerHasItemWithHimCondition(
						"book_blue")), ConversationStates.IDLE,
		...
		npc.add(
				...
				new SpeakerNPC.ChatCondition() {
					...
					public boolean fire(Player player, Sentence sentence,
						...
						return player.hasQuest(QUEST_SLOT)
								&& (player.getQuest(QUEST_SLOT).equals(
										"seeking_book") || player.getQuest(
										QUEST_SLOT).equals("got_book"))
	...
	private void offerKnifeStep() {
		...
		npc.add(
				...
				"I'll make an obsidian knife if you can get the gem which makes the blade. Bring a "
						+ FISH + " so that I can make the bone handle, too.",
				null);
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				new SpeakerNPC.ChatCondition() {
					...
					public void fire(Player player, Sentence sentence,
						...
						player.addXP(10000);
						Item knife = StendhalRPWorld.get().getRuleManager().getEntityManager().getItem(
								"obsidian_knife");
