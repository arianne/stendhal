				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						String food = player.getQuest(QUEST_SLOT);
					        npc.say("Thank you! I hope it doesn't take too long to collect. Don't forget to say '" + food + "' when you have it.");
						//player.setQuest(QUEST_SLOT, food);
						player.addKarma(5.0);
						// set food to null?
					}
				});
		//Player says no. they might get asked to bring a different food next time but they've lost karma.
		npc.add(ConversationStates.QUEST_OFFERED,
			...
			new SpeakerNPC.ChatAction() {
				@Override
				    public void fire(Player player, String text, SpeakerNPC npc) {
						...
						npc.say("Thanks. Try asking at a library for a 'gem_book'.");
...
private void offerKnifeStep() {
	...
	SpeakerNPC npc = npcs.get("Alrak");
		...
		// Here because of the random response from different actions bug, we define the greeting message for all cases not covered so far:
	...
	npc.add(ConversationStates.IDLE,
	        ...
	        ConversationStates.ATTENDING,
"How did you get down here? I usually only see #kobolds.", 
			    null);
