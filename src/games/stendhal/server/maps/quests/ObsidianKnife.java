					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						if (!player.hasQuest(QUEST_SLOT)
								|| /* player.hasQuest(QUEST_SLOT)&& */player
										...
										.equals("rejected")) {
							npc
									.say("You know, it's hard to get food round here. I don't have any #supplies for next year.");
							npc
									.setCurrentState(ConversationStates.QUEST_ITEM_QUESTION);
						} else if (player.isQuestCompleted(QUEST_SLOT)) {
							npc
									.say("I'm inspired to work again! I'm making things for Wrvil now. Thanks for getting me interested in forging again.");
						} else if (player.hasQuest(QUEST_SLOT)
								&& player.getQuest(QUEST_SLOT).equals(
										"food_brought")) {
							npc
									.say("Now I'm less worried about food I've realised I'm bored. There's a #book I'd love to read.");
							npc
		...
		npc.add(ConversationStates.QUEST_OFFERED,
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						...
						npc
								.say("Thank you! I hope it doesn't take too long to collect. Don't forget to say '"
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				new SpeakerNPC.ChatCondition() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						...
						npc
								.say("Great! I think I'll read this for a while. Bye!");
						player.addXP(500);
				...
				new SpeakerNPC.ChatCondition() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						String[] tokens = player.getQuest(QUEST_SLOT)
								.split(";");
						long delay = REQUIRED_DAYS * 60 * 60 * 24 * 1000; // milliseconds
																			// in
																			// REQUIRED_DAYS
																			// days
						...
						if (timeRemaining > 0L) {
							npc
									.say("I haven't finished reading that book. Maybe I'll be done in "
											+ TimeUtil
						...
						npc
								.say("I've finished reading! That was really interesting. I learned how to make a special #knife from #obsidian.");
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				new SpeakerNPC.ChatCondition() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						...
						player.drop(FISH);
						npc
								.say("You found the gem for the blade and the fish bone to make the handle! I'll start work right away. Come back in "
						...
						new SpeakerNPC.ChatCondition() {
							...
							public boolean fire(Player player, String text,
								...
								return player.hasQuest(QUEST_SLOT)
										&& player.getQuest(QUEST_SLOT).equals(
												"knife_offered")
										&& !(player.isEquipped("obsidian") && player
												.isEquipped(FISH));
							}
						},
						ConversationStates.ATTENDING,
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				new SpeakerNPC.ChatCondition() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						...
						if (timeRemaining > 0L) {
							npc
									.say("I haven't finished making the knife. Please check back in "
											+ TimeUtil
													.approxTimeUntil((int) (timeRemaining / 1000L))
											+ ".");
							return;
						...
						npc
								.say("The knife is ready! You know, that was enjoyable. I think I'll start making things again. Thanks!");
						player.addXP(10000);
				...
				new SpeakerNPC.ChatCondition() {
					...
					public boolean fire(Player player, String text,
						...
						return !player.hasQuest(QUEST_SLOT)
								...
								|| (player.hasQuest(QUEST_SLOT) && NOT_COVERED_LIST
										.contains(player.getQuest(QUEST_SLOT)));
					}
				}, ConversationStates.ATTENDING,
