	...
	private void FixRingStep() {
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						Item emeraldRing = player
								.getFirstEquipped("emerald_ring");
						if (emeraldRing != null
								&& emeraldRing.getInt("amount") > 0) {
							// ring is not broken so he just lets player know
							// where it can be fixed
							npc
									.say("Hello. That's a rare emerald on your ring. If it gets broken, come to me to fix it.");
							npc.setCurrentState(ConversationStates.ATTENDING);
						} else {
							// notices ring is broken
							npc
									.say("What a pity, your emerald ring is broken. I can fix it, for a #price.");
					...
					public boolean fire(Player player, String text,
						...
						return !player.isEquipped("emerald_ring")
								&& (!player.hasQuest(QUEST_SLOT) || player
										.isQuestCompleted(QUEST_SLOT))
								&& (!player.hasQuest(MARRIAGE_QUEST_SLOT) || player
										.hasQuest(MARRIAGE_QUEST_SLOT)
										&& !player
												.getQuest(MARRIAGE_QUEST_SLOT)
												.startsWith("forging;"));
					}
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				new SpeakerNPC.ChatCondition() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						...
						if (timeRemaining > 0L) {
							npc
									.say("I haven't finished fixing your ring. Please check back in "
											+ TimeUtil
						...
						npc
								.say("I'm pleased to say, your ring is fixed! It's good as new now.");
						player.addXP(500);
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						if ((player.isEquipped("gold_bar", REQUIRED_GOLD))
							...
							npc
									.say("Okay, that's all I need to fix the ring. Come back in "
											+ REQUIRED_MINUTES
							...
							npc
									.say("Come back when you have the money, the gem and the gold.");
							/*
		...
		npc.add(ConversationStates.QUEST_ITEM_BROUGHT,
				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text, SpeakerNPC npc) {
						npc
								.say("No problem, just come back when you have the money, the emerald, and the gold.");
						/*
