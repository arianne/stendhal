...
public class CloaksForBario extends AbstractQuest {
	...
	private void step_3() {
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES, new SpeakerNPC.ChatCondition() {
			...
			@Override
			public void fire(Player player, String text, SpeakerNPC engine) {
				engine.say("Hi again! I still need " + player.getQuest(QUEST_SLOT) + " blue elven "
				        + Grammar.plnoun(MathHelper.parseInt(player.getQuest(QUEST_SLOT)), "cloak")
		...
		}, ConversationStates.ATTENDING, "Welcome! Thanks again for those cloaks.", null);

		// player says he doesn't have any blue elf cloaks with him
		npc.add(ConversationStates.QUESTION_1, "no", null, ConversationStates.ATTENDING, "Too bad.", null);

		// player says he has a blue elf cloak with him
		npc.add(ConversationStates.QUESTION_1, ConversationPhrases.YES_MESSAGES, null, ConversationStates.ATTENDING,
		        null, new SpeakerNPC.ChatAction() {
				        ...
				        if (player.drop("elf_cloak_+2")) {
					        ...
					        if (toBring > 0) {
						        ...
						        engine.say("Thank you very much! Do you have another one? I still need "
						                + Grammar.quantityplnoun(toBring, "cloak") + ".");
