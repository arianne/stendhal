				...
				new SpeakerNPC.ChatAction() {
					...
					public void fire(Player player, String text,
						...
						if (!player.hasQuest(QUEST_SLOT)) {
							engine
									.say("I once forged the most powerful of swords. I can do it again for you. Are you interested?");
						} else if (player.isQuestCompleted(QUEST_SLOT)) {
							engine
									.say("Oh! I am so tired. Look for me later. I need a few years of relaxing.");
							engine
					...
					public void fire(Player player, String text,
						...
						if (!missingSomething && neededIron > 0) {
							if (!player.isEquipped("iron", neededIron)) {
								...
								engine
										.say("I cannot #forge it without the missing "
						...
						if (!missingSomething && neededWoodLogs > 0) {
							if (!player.isEquipped("wood", neededWoodLogs)) {
								...
								engine
										.say("How do you expect me to #forge it without missing "
												+ Grammar.quantityplnoun(
														neededWoodLogs,
														"wood log")
												+ " for the fire?");
						...
						if (!missingSomething && neededGoldBars > 0) {
							if (!player.isEquipped("gold_bar", neededGoldBars)) {
								int amount = player
										.getNumberOfEquipped("gold_bar");
								if (amount > 0) {
									player.drop("gold_bar", amount);
									neededGoldBars -= amount;
								}
								engine
										.say("I must pay a bill to spirits in order to cast the enchantment over the sword. I need "
												+ Grammar.quantityplnoun(
														neededGoldBars,
														"gold bar") + " more.");
								missingSomething = true;
						...
						if (!missingSomething && neededGiantHearts > 0) {
							if (!player.isEquipped("giant_heart",
									neededGiantHearts)) {
								int amount = player
										.getNumberOfEquipped("giant_heart");
								if (amount > 0) {
									player.drop("giant_heart", amount);
									neededGiantHearts -= amount;
								}
								engine
										.say("It is the base element of the enchantment. I need "
												+ Grammar.quantityplnoun(
														neededGiantHearts,
														"giant heart")
												+ " still.");
						...
						if (player.hasKilled("giant") && !missingSomething) {
							...
							if (!player.hasKilled("giant") && !missingSomething) {
								engine
										.say("Did you really get those giant hearts yourself? I don't think so! This powerful sword can only be given to those that are strong enough to kill a #giant.");
							}
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
				new SpeakerNPC.ChatCondition() {
					...
					public void fire(Player player, String text,
						...
						String[] tokens = player.getQuest(QUEST_SLOT)
								.split(";");
						long delay = REQUIRED_TIME * 60 * 1000; // minutes ->
																// milliseconds
						...
						engine
								.say("I have finished forging the mighty immortal sword. You deserve this. Now I'm going to have a long rest, so, goodbye!");
						player.addXP(15000);
