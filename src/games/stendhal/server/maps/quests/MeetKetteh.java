...
public class MeetKetteh extends AbstractQuest {
	...
	private void step1() {
		...
		npc.add(ConversationStates.ATTENDING, "no", null, ConversationStates.IDLE,
		        "If you don't put on some clothes and leave, I shall scream!", null);

		npc.add(ConversationStates.ATTENDING, "shout", null, ConversationStates.ATTENDING, null,
		        new SpeakerNPC.ChatAction() {
			        ...
			        public void fire(Player player, String text, SpeakerNPC engine) {
				        if (player.hasQuest("Ketteh") && player.getQuest("Ketteh").equals("seen_naked")) {
					        engine
					                .say("I am glad to see you've acquired some clothes. There really is no need for walking around naked.");
				        } else {
					        engine
					                .say("Sometimes naked people pass by; it makes me very angry. They are bringing down the tone of the whole place!");
				        }
			        }
		        });
	}
