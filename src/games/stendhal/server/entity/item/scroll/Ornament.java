...
/**
 * Represents the ornament that takes the player to the northpole zone,
 * after which it will teleport player to a random location in 0_semos_plains_s.
 */
public class OrnamentScroll extends TimedTeleportScroll {

	private static final long DELAY = 6 * MathHelper.MILLISECONDS_IN_ONE_HOUR;

	/**
	 * Creates a new timed marked OrnamentScroll scroll.
	 *
	 * @param name
	 * @param clazz
	 * @param subclass
	 * @param attributes
	 */
	public OrnamentScroll(final String name, final String clazz, final String subclass,
	 ...
	 *            item to copy
	 */
	public OrnamentScroll(final OrnamentScroll item) {
		super(item);
	}

	@Override
	protected boolean useTeleportScroll(final Player player) {
		final String QUEST_SLOT = "northpole";
		long lastuse = -1;
		if (player.hasQuest(QUEST_SLOT)) {
			...
			if (timeRemaining > 0) {
				// player used the ornament within the last DELAY hours
				...
				player.sendPrivateText("You are not allowed to overuse the northpole.");
			...
			// players can only obtain the ornament from xxxxx who stores the time bought in quest slot
