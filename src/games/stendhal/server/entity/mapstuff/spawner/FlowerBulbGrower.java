 ...
 * fruit.
 * 
 * Fruit is only grown when FlowerBulbGrower is in the same place as an entity in
...
public class FlowerBulbGrower extends VegetableGrower {
	...
	private final String[] description = {
			"You see a bulb which has just been planted.",
	...
	/**
	 * Constructor for loading FlowerBulbGrower from the stored zone used by
	 ...
	 *            the item to grow
	 */
	public FlowerBulbGrower(final RPObject object, final String itemname) {
	 ...
	 * Constructor.
	 * 
	 * Default FlowerBulbGrower produces zantedeschia flowers.
	 */
	public FlowerBulbGrower() {
		this(GROWER_NAME);
		store();
	}

	/**
	 * Constructor of a FlowerBulbGrower growing an item with the name specified in
	 * infostring.
	 * 
	 * @param infoString
	 ...
	 * 
	 */
	public FlowerBulbGrower(final String infoString) {
		super(infoString);
		setMaxRipeness(4);
		meanTurnsForRegrow = GROW_TIME_TURNS;
		store();
	...
	public boolean isOnFreeFertileGround() {
		if (this.getZone() == null) {
			...
			for (Entity entity : zone.getEntitiesAt(getX(), getY())) {
				if (entity instanceof FlowerBulbGrower) {
					if (!equals(entity)) {
