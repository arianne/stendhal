...
public class StendhalNavigable implements Navigable
{
    ...
    protected static final double DEFAULT_MAX_DISTANCE = 40.0;
     ...
     *            The goal
     */
    public StendhalNavigable(Entity entity, StendhalRPZone zone, int startX, int startY,
        Rectangle2D destination)
    {

        this(entity, zone, startX, startY, destination, DEFAULT_MAX_DISTANCE, false);
    }
     ...
     */
    public StendhalNavigable(Entity entity, StendhalRPZone zone, int startX, int startY,
        Rectangle2D destination, boolean checkEntities)
    {

        this(entity, zone, startX, startY, destination, DEFAULT_MAX_DISTANCE, checkEntities);
    }
        ...
        Rectangle2D destination, double maxDist, boolean checkEntities)
    {
        ...
        double startF =
            getHeuristic(startX, startY, (int) destination.getCenterX(), (int) destination
                .getCenterY());
    ...
    {
        boolean result = !zone.simpleCollides(entity, node.x, node.y);
    ...
    /**
     * calculates the cost for the move from the parent to the child costs are
     * allway 1 in stendhal
     * 
     * @param parent
     *            the parent node
     * @param child
     ...
     */
    public double getHeuristic(PathFinderNode node1, PathFinderNode node2)
    {
        ...
        if (node.f < maxDistance)
        {
            for (int i = -1; i < 2; i++)
            {
                for (int j = -1; j < 2; j++)
                {
                    ...
                    {
                        PathFinderNode childNode = new PathFinderNode(x + i, y + j);
