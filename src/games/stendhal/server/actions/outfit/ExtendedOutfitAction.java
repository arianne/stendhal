...
import static games.stendhal.common.constants.Actions.OUTFIT_EXTENDED;
...
/**
 * Changes the extended outfit features (currently mouth and eyes) for the
 * player.
 * 
 * FIXME: Can this be integrated into OutfitAction?
 */
public class ExtendedOutfitAction implements ActionListener {
	private static String COLOR_MAP = "outfit_colors";
	
	public static void register() {
		CommandCenter.register(OUTFIT_EXTENDED, new ExtendedOutfitAction());
	}
	...
	public void onAction(final Player player, final RPAction action) {
		if (action.has(VALUE)) {
			...
			if (outfit.isChoosableByPlayers()) {
				new GameEvent(player.getName(), OUTFIT_EXTENDED, action.get(VALUE)).raise();
