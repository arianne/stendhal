...
public class Pathfind {
	...
	public boolean NewPath(CollisionDetection collisiondetection,
			int x_inicial, int y_inicial, int x_final, int y_final,
			Rectangle search_area2) {

		// System.out.println("PATHFIND: " + x_inicial + " " + y_inicial + " "
		// +x_final + " "+ y_final + " " +collisiondetection.toString());
		 ...
		 * System.out.println("DESTINO NO CAMINABLE: " + x_inicial + " " +
		 * y_inicial + " " +x_final + " "+ y_final + " " ); return false;
		 *  }
		 */
		...
		nodeRegistry.put(nodo_ini.x + nodo_ini.y
				* collisiondetection.getWidth(), nodo_ini);
		...
		do {
			...
			if (lista_abierta.size() == 0) {
				...
				// lista_cerrada.size());// elapsed time: " + computation_time);
				return false;
			}
			...
			nodeRegistryclose.put(nodo_Fm.x + nodo_Fm.y
					* collisiondetection.getWidth(), nodo_Fm);

			// System.out.println("Elementos:"+lista_abierta.size()+":"+lista_cerrada.size());

			// Se ha llegado al final
			if ((nodo_Fm.x == x_final) && (nodo_Fm.y == y_final)) {
				break;
			}
			...
			for (y_tmp = nodo_Fm.y - 1; y_tmp <= nodo_Fm.y + 1; y_tmp++) {
				for (x_tmp = nodo_Fm.x - 1; x_tmp <= nodo_Fm.x + 1; x_tmp++) {
					...
					if ((y_tmp != nodo_Fm.y) && (x_tmp != nodo_Fm.x)) {
						continue;
					}

					// // Si no es transitable o si esta en la lista cerrada,
					// ignoralo.
					...
					if (colision(collisiondetection, x_tmp, y_tmp) == 0) {
						int manhatan = 10 * (Math.abs(x_tmp - x_final) + Math.abs(y_tmp
								- y_final));

						Node nodo_UP;
						if (Math.abs(x_tmp - nodo_Fm.x) == 1
								&& Math.abs(y_tmp - nodo_Fm.y) == 1) {
							nodo_UP = new Node(x_tmp, y_tmp, nodo_Fm.G + 14,
									manhatan);
						} else {
							int patata = 0;
							...
							nodo_UP = new Node(x_tmp, y_tmp, nodo_Fm.G + 10
									- patata, manhatan);

						}
						nodo_UP.padre = nodo_Fm;
						// System.out.println("ADYACENTE:"+x_tmp+":"+y_tmp + " G
						// " +nodo_UP.G + " H " + nodo_UP.H + " H " +
						// nodo_UP.F);

						Node temp = nodeRegistry.get(nodo_UP.x + nodo_UP.y
								* collisiondetection.getWidth());

						if (temp != null) {
							// //Si ya esta en la lista abierta, comprueba si el
							// camino para ese
							// es mejor usando el coste G como baremo. Un coste
							// G menor significa
							// que este es un mejor camino. Si es asi, cambia el
							// padre del cuadrado
							// al cuadro actual y recalcula G y F del cuadro. Si
							// estas manteniendo la
							// lista abierta por orden de puntuacion F, podrias
							// necesitar reordenar la
							...
							// //Si no esta en la lista abierta, anyadelo a la
							// lista abierta.
							// Haz que el cuadro actual sea el padre de este
							// cuadro. Almacena los
							// costes F, G y H del cuadro.
							// lista_abierta.add(nodo_UP);
							lista_abierta.offer(nodo_UP);
							nodeRegistry.put(nodo_UP.x + nodo_UP.y
									* collisiondetection.getWidth(), nodo_UP);
						}
					}
				}
			}
		...
		while (i.hasNext()) {
			...
			// + temp.H + " "+ temp.padre.x +":" + temp.padre.y);
		}
		...
		for (int j = lista_cerrada.size() - 1; j >= 0; j--) {
			...
			if ((petiX == temp.x) && (petiY == temp.y)) {
				...
				// H " + temp.H + " "+ temp.padre.x +":" + temp.padre.y);
				petiX = temp.padre.x;
				petiY = temp.padre.y;
