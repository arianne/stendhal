 ...
 * crappy code XD
 * TODO: Check if colision() method should be renamed to
 * collision() to prevent double spellings
 * TODO: Check if Reinice() method should be renamed to
...
public class Pathfind {
	...
	public boolean NewPath (CollisionDetection collisiondetection,int initial_x, int initial_y, int final_x, int final_y, Rectangle search_area2){


	//	System.out.println("PATHFIND: " + initial_x + " " + initial_y + " " +final_x + " "+ final_y + " " +collisiondetection.toString());
		...
		nodeRegistry.put(ini_node.x + ini_node.y * collisiondetection.getWidth(), ini_node);

		do{
			// THERE IS NO PATH
			if (open_list.size()==0) {
				//	computation_time = System.currentTimeMillis() - computation_time;
			//	System.out.println("THERE IS NO PATH!! "+ closed_list.size());// elapsed time: " + computation_time);
				return false;
			}
			...
			nodeRegistryclose.put(node_Fm.x + node_Fm.y * collisiondetection.getWidth(), node_Fm);

			//	System.out.println("Elements:"+open_list.size()+":"+closed_list.size());

			// The end has been reached
			if ((node_Fm.x==final_x)&&(node_Fm.y==final_y)) break;
			...
			for (y_tmp = node_Fm.y-1; y_tmp <= node_Fm.y+1; y_tmp++){
				for (x_tmp = node_Fm.x-1; x_tmp <= node_Fm.x+1; x_tmp++){
					...
					if (y_tmp==node_Fm.y)
						if (x_tmp==node_Fm.x)
							continue;
					if ((y_tmp!=node_Fm.y)&&(x_tmp!=node_Fm.x))	continue;
...
//					Otherwise do the following.
					...
					if (colision( collisiondetection,x_tmp,y_tmp)==0){
						...
						Node  node_UP;
						if (Math.abs(x_tmp-node_Fm.x) == 1 && Math.abs(y_tmp-node_Fm.y) == 1)
							node_UP = new Node(x_tmp,y_tmp,node_Fm.G+14, manhattan);
						else	{
							int potato = 0;
						...
						//	System.out.println("ADJACENT:"+x_tmp+":"+y_tmp + " G " +node_UP.G + " H " + node_UP.H + " H " + node_UP.F);

						Node temp = nodeRegistry.get(node_UP.x + node_UP.y * collisiondetection.getWidth());
...
//							that this is a better path. If so, change the parent of the square
...
//							 F, G, and H costs of the square.
							...
							nodeRegistry.put(node_UP.x + node_UP.y * collisiondetection.getWidth(), node_UP);
						}
					}
				}
			}
		...
		{
			temp=(Node)i.next();
			//		System.out.println("E:"+temp.x+":"+temp.y + " G " +temp.G + " H " + temp.H + " "+ temp.parent.x +":" + temp.parent.y);
		}
		...
		for (int j=closed_list.size()-1; j>=0;j--){
			...
			if ((petiX==temp.x)&&(petiY==temp.y)){
				...
				//	System.out.println("S:"+temp.x+":"+temp.y + " G " +temp.G + " H " + temp.H + " "+ temp.parent.x +":" + temp.parent.y);
				petiX = temp.parent.x;
				petiY = temp.parent.y;
	...
	//	System.out.println("PATH FOUND!! " + closed_list.size());//elapsed time: " + computation_time);
