 ...
 * 
 * This Software is provided as it is and is published under 
 * the GPL-License. No warranty is provided. Building systems 
 * and portals based on this system is on your own risk.
 * 
 * @author Ken Werner
 * modulname: Client
 * contact:	wernerk@fh-brandenburg.de
 *
 * ScrollerPanel can be used for displaying a scrolling text (as in movie credits).
 *
...
//	private static final Logger logger = Log4J.getLogger(ScrollerPanel.class);
	...
	/**
	 * creates an ScrollerPane wich scrolls the given text and using defaults for the other attributes.
	...
	/**
	 * creates an ScrollerPane wich scrolls the given text and uses the given attributes
	 * @param text the text araay whis should be scrolled - one string per line is scrolled
	 ...
	 */
	public ScrollerPanel( String[] text, Font font, int lineSpacing, Color textColor, Color backgroundColor, int scrollSpeed){
		this.text = text;
		this.font = font;
	...
	public void paint(Graphics g ) {
		...
		for (int i = 0, n = text.length; i < n; i++) {
			...
			g2d.drawString( text[i], this.getWidth() / 2 - width / 2, textPos + ((metrics.getHeight() + lineSpacing) * i));
		}
	}
