...
public class TileSprite implements Sprite {
	 ...
	 *
	 * <strong>NOTE: The sprite passed is not copied, and must not
	 * be modified while this instance exists (unless you are sure
	 * you know what you are doing).</strong>
	 *
	 * @param	sprite		The source sprite.
	 *
	 * @throws	IllegalArgumentException
	 *				If the region if beyond the source
	 *				sprite's bounds.
	 */
	public TileSprite(final Sprite sprite, int x, int y, int width, int height) throws IllegalArgumentException {
		this(sprite, x, y, width, height, createReference(sprite, x, y, width, height));
	}
	 ...
	 *				sprite's bounds.
	 */
	public TileSprite(final Sprite sprite, int x, int y, int width, int height, Object reference) throws IllegalArgumentException {
		this.sprite = sprite;
		this.x = x;
		this.y = y;
		this.width = width;
	 ...
	 *
	 */
	public static Object createReference(final Sprite sprite, int x, int y, int width, int height) {
	...
	/**
	 * Copy the sprite. This does not do a deep copy, so the underlying
	 * sprite it is made of are shared.
	 *
	...
	public void draw(final Graphics g, final int destx, final int desty, final int x, final int y, final int w, final int h) {
		sprite.draw(g, destx, desty, x + this.x, y + this.y, Math.min(w, width), Math.min(h, height));
	}
