...
/** The rollover color. */
	...
	/** The listeners. */
	private transient List<ColorListener> listeners = new ArrayList<ColorListener>();
	...
	public Dimension getPreferredSize() {
		...
		return new Dimension((colorSize + colorSpacing) * colors[0].length + (2 * margin) - colorSpacing,
				(colorSize + colorSpacing) * colors.length + (2 * margin) - colorSpacing);
	}
	...
	private Color getColor(int x, int y) {
		...
		if ((col < 0) || (col >= colors[0].length) || (row < 0) || (row >= colors.length)) {
			return null;
		} else {
	...
	protected void paintComponent(Graphics g) {
		...
		for (int y = 0; y < colors.length; y++) {
			for (int x = 0; x < colors[0].length; x++) {
				...
				g.setColor(color);
				g.fillRect(x * (colorSize + colorSpacing) + margin, y * (colorSize + colorSpacing) + margin,
						colorSize, colorSize); // draw rollover?
				//if (colorSpacing > 0) {
				if (color == rolloverColor) {
					g.setColor(ROLLOVER_COLOR);
					g.fillRect(x * (colorSize + colorSpacing) + margin,
							y * (colorSize + colorSpacing) + margin, colorSize, colorSize);
				}
			}
		}
	}
