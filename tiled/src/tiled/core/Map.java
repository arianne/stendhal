...
public class Map extends MultilayerPlane
{
   ...
   * TODO: better use a java.io.File-object
   */
  ...
  /**
   * Adds a change listener. The listener will be notified when the map changes
   * in certain ways.
   * 
   * @param l
   ...
   * from the map layers. A {@link MapChangedEvent} is fired when all processing
   * is complete.
   * 
   * @param s
   *          TileSet to remove
   ...
   * @return boolean <code>true</code> if the point lies within the bounds of
  ...
  /**
   * Returns the maximum tile height. This is the height of the highest tile in
   * all tilesets or the tile height used by this map if it's smaller.
   * 
  ...
  {
    return "Map[" + widthInTiles + "x" + heightInTiles + "x" + getTotalLayers() + "][" + tileWidth + "x" + tileHeight + "]";
  }
