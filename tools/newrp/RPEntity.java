...
public class RPEntity {
	...
	/**
	 * Which Race this entity belongs to.
	 * Each race has different racial bonus.
	 */
	Race race;
	...
	/**
	 * How offensive/defensive the entity is, with 0 as most
	 * defensive and 1 as most offensive.
	 */
	float attitude;
	 ...
	 * @param item weapon, shield or armor.
	 */
	...
	/**
	 * Generate random stats for entity such that all the basic attributes sum 60.
	 */
	public void rand() {
		...
		while (sum != 60) {
			...
			sum = strengh + dextrexity + agility + constitution + inteligence + wisdom;
		}
	 ...
	 *
	 * @param newlevel the new level the entity will have.
	 */
	public void level(int newlevel) {
		int diff = newlevel - level;
		level = newlevel;

		basehp = basehp + diff * (int) (constitution * constitution * type.constitution / 10.0);
		basemp = basemp + diff * (int) (((inteligence + wisdom) / 2.0) * ((type.inteligence + type.wisdom) / 2.0));
		weight = weight
				+ diff
				* (int) ((type.strengh + type.constitution + type.agility) * (strengh + constitution + agility) / (3.0 * 3.0));
	...
	/**
	 * This method is used to cause damage to target of the given type and
	 ...
	 * It is a measure of how good are the hits we can do.
	 * Doing a hit is relatively simply as long as you are strong enough to handle
	 * the weapon with your dextrexity.
	 */
	private float getHitQuality(float attitude) {
		float quality= dextrexity * (strengh / weapon.weight) * attitude * 100 / 256f;
		return quality;
	}

	/**
	 * Roll a dice to see if hit was successfull.
	 * We roll 2D6 and substract our hit and our level/10.
	 * If the result if 0 or above, we failed to do the hit.
	 *
	 * @param attitude
	 * @return
	 */
	...
	/**
	 * Returns how many turns we need to spend between attacks.
	 * It is related to how much armor weights ( the more the worse ) and how
	 * much is our agility and our strengh ( the more agile and the stronger
	 * we are, the better. )
	 *
	 * @return
	 */
	...
	/**
	 * The higher the better.
	 * It is a measure of how good are the dodge we can do.
	 * Doing a dodge is hard, even worse when we have armor.
	 * Don't expect to dodge with your full plate armor
	 */
	...
	/**
	 * Roll a dice to see if dodge was successfull.
	 * We roll 4D6 and substract our dodge and our level/20.
	 * If the result if 0 or above, we failed to do the dodge.
	 *
	 * @param attitude
	 * @return
	 */
	...
	/**
	 * Returns how many turns we need to spend between shield defense.
	 * It is related to how much shield weights ( the more the worse ) and how
	 * much is our dextrexity and our strengh ( the better we handle the shield and the stronger
	 * we are, the better. )
	 *
	 * @return
	 */
	...
	/**
	 * The higher the better.
	 * It is a measure of how good are we handle the shield.
	 */
	...
	/**
	 * Once damage is done check how much of it is absorbed by shield.
	 * Damage absorbed is always between two not rand values min and max, then damage absorbed is randomly choosen between them.
	 *
	 * @param damage
	 * @param attitude
	 */
	private int shieldAbsorb(DamageType type, int amount, float attitude) {
		...
		float min_coef = getShieldQuality(attitude) * ((level * level / 10000.0f) + level / 40f + 0.25f);
		float max_coef = getShieldQuality(attitude) * ((level * level / 2500.0f) + level / 10f + 1);
		...
		for (Effect effect : shield.protect) {
			if(effect instanceof DamageEffect) {
				...
				if(absorb.type==type) {
					...
					//System.out.println("shield removes ["+min+","+max+"]->"+shieldtakes);
	...
	/**
	 * Once damage is done check how much of it is absorbed by armor.
	 * Damage absorbed is always between two not rand values min and max, then damage absorbed is randomly choosen between them.
	 *
	 * @param damage
	 */
		...
		for (Effect effect : armor.protect) {
			if(effect instanceof DamageEffect) {
				...
				if(absorb.type==type) {
					...
					//System.out.println("armor removes ["+min+","+max+"]->"+armorTakes);
	...
	public float getCastQuality(Spell spell, float attitude) {
		float quality=((inteligence+wisdom)/2 -(level-spell.level)) * attitude;
		return quality;
	}
	...
	public void cast(Spell spell, RPEntity target, int turn) {
		/*
		 * Each spell takes a time to reload.
		 * The harder the spell the more time until you can cast another again.
		 */
		if(turn > turnToCastAgain) {
			...
			if(dice.success()) {
				...
				/*
				 * The spell fails.
				 * There is a 10% + 3 for each level of difference of backfiring the spell.
				 */
...
enum School {
	...
	School(double str, double dex, double agi, double con, double inte, double wis) {
		this.strengh = str;
		this.dextrexity = dex;
		this.agility = agi;
		this.constitution = con;
